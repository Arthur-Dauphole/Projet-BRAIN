...
class TestSuite:
    ...
    def get_report_string(self) -> str:
        """
        Generate the formatted test report as a string.
        """
        ...
        lines.append(f"Total tests run:     {report['total_tests']}")
        lines.append(f"Tests passed:        {report['passed']} ✓")
        lines.append(f"Tests failed:        {report['failed']} ✗")
        ...
        if report['failed'] > 0:
            ...
            lines.append(f"Rectangle detection failures: {failures['rectangles']}")
            lines.append(f"Line detection failures:      {failures['lines']}")
            lines.append(f"Exception failures:           {failures['exceptions']}")

            lines.append(f"\n{'='*70}")
            lines.append(f"FAILED TESTS DETAILS")
            lines.append(f"{'='*70}\n")

            for i, failure in enumerate(report['failed_tests'], 1):
                lines.append(f"{i}. {failure['test_name']} ({failure['test_id']})")
        ...
        if report['pass_rate'] == 100:
            ...
            lines.append("⚠️  MOSTLY PASSING (some failures)")
        else:
            lines.append("❌ MULTIPLE FAILURES DETECTED")
        lines.append(f"{'='*70}\n")
    ...
    def visualize_test(self, test_id: str, save_path: str = None):
        ...
        test_case = next((tc for tc in self.test_data['test_grids'] if tc['id'] == test_id), None)
        ...
        grid = np.array(test_case['grid'])
        analysis = self.engine.analyze_grid(grid, verbose=False)
        all_shapes = [shape for shapes in analysis['detected_shapes'].values() for shape in shapes]
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Run geometric detection tests on ARC-like grids")
    
    parser.add_argument('--json', '-j', default='arc_test_grids.json', help='Path to test grids JSON file')
    parser.add_argument('--verbose', '-v', action='store_true', help='Print detailed output for each test')
    parser.add_argument('--filter', '-f', type=str, default=None, help='Filter tests by tag')
    parser.add_argument('--visualize', '-viz', action='store_true', help='Generate visualizations for failed tests')
    parser.add_argument('--visualize-all', '-viz-all', action='store_true', help='Generate visualizations for ALL tests')
    parser.add_argument('--test-id', '-t', type=str, default=None, help='Visualize a specific test by ID')
    parser.add_argument('--export', '-e', type=str, default=None, help='Export results to JSON file')
    parser.add_argument('--output-dir', '-o', type=str, default='test_failures', help='Base output directory for visualizations')
    ...
    if args.visualize or args.visualize_all:
        ...
        report_content = suite.get_report_string()
        report_path = timestamped_dir / "test_report.txt"
        timestamped_dir.mkdir(exist_ok=True, parents=True) # Ensure directory exists before writing
    ...
    if len(sys.argv) == 1:
        ...
        if suite.load_tests():
            ...
            if report.get('failed', 0) > 0:
                # --- AUTOMATIC GENERATION OF REPORT AND VISUALIZATIONS ---
                print("\nDetected failures. Automatically generating report and visualizations...")
                script_dir = Path(__file__).parent
                timestamp = datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
                output_dir_path = script_dir / f"test_failures_{timestamp}"
                
                # Save the text report
                report_content = suite.get_report_string()
                report_path = output_dir_path / "test_report.txt"
                ...
                # Generate visualizations
                suite.visualize_all_failed_tests(output_dir=str(output_dir_path))
