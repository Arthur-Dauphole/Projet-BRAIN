"""
SYSTÈME COMPLET DE RÉSOLUTION DE GRILLES ARC AVEC PROPAGATIONS MULTIPLES ET DÉTECTION AVANCÉE DE COULEURS
- Détection fine des formes géométriques
- Extraction des règles avec propagations horizontales et verticales
- Détection avancée des changements de couleurs (bijections, patterns)
...
class AdvancedGridRuleExtractor:
    ...
    def __init__(self, use_shape_detection: bool = True):
        ...
        self.position_based_changes = {}  # changements basés sur la position (CODE 2)
        self.color_pairs = set()  # stocke toutes les paires de couleurs (CODE 2)
    ...
    def _collect_color_data(self, input_grid: List[List[int]], 
                          output_grid: List[List[int]]) -> None:
        """Collecte les données de couleur avec détection de patterns."""
        h = len(input_grid)
        w = len(input_grid[0])
        
        # Vérifier les patterns par ligne/colonne
    ...
    def _build_color_mapping_advanced(self):
        """Construit le mapping de couleurs avancé (CODE 2 amélioré)."""
    ...
    def _analyze_transformations_with_propagation(self, input_grid: List[List[int]], 
                                                 output_grid: List[List[int]]) -> None:
        """Analyse les transformations avec détection de propagations (CODE 1 amélioré)."""
        h = len(input_grid)
        w = len(input_grid[0])
...
class AdvancedGridTransformer:
    """Applique les règles de transformation avec support des propagations multiples (CODE 1 amélioré)."""
...
def main(visualize: bool = True, json_file: str = "grid_data.json", 
    ...
    output_file = "grid_results_hybrid.json"
...
if __name__ == "__main__":
    ...
    print("2. Extraction avancée des changements de couleur (bijections, patterns)")
    print("3. Détection des propagations horizontales et verticales")
    print("4. Application complète des règles (changements, propagations, connexions, translations)")
    print("5. Visualisation améliorée avec informations de formes")
    print("=" * 70)
